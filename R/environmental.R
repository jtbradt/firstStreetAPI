    ...
    resp <- lapply(lookup.arg, function(l) {
        ...
        if (api == "sea-level") {
            ...
            tide.resp <- lapply(tide, function(s) {
                ...
                if (geometry == TRUE) {
                  ...
                  if (class(geometry.data)[1] != "character") {
                    ...
                    slr.historic <- sf::st_as_sf(cbind(data.table::data.table(slr.historic), geometry.data))
                    slr.projected <- sf::st_as_sf(cbind(data.table::data.table(slr.projected), geometry.data))
                  }
                }
            ...
            return <- lapply(1:2, function(x) {
                ...
                temp<- data.table::rbindlist(temp, fill = TRUE)
                temp$fsid <- parsed[[1]]
